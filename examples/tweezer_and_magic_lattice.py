...
x = np.linspace(
    -4, 4, 200
)  # np.concatenate(([0],np.linspace(-0.01,-2,20),np.linspace(0.01,2,20)))
y = np.linspace(
    -2, 2, 42
)  # np.concatenate(([0],np.linspace(-0.01,-2,20),np.linspace(0.01,2,20)))
...
for ci, ax_pair in enumerate(axs.T):
    ...
    axis_var = [x, y, z][ci]
    axis_slice = tuple(
        (this_expand_around[d] if d != ci else axis_var for d in range(3))
    )
    ...
    wf_space, dxwfs = np.linspace(
        centre_position_um - 1.3, centre_position_um + 1.3, 200, retstep=True
    )
    wf = (mass * u * 2 * np.pi * trap_freq * 1e6 / (np.pi * scipy.constants.hbar)) ** (
        0.25
    ) * np.exp(
        -(
            mass
            ...
            * trap_freq
            * 1e6
            * ((wf_space - centre_position_um) * 1e-6) ** 2
        )
        / (2 * scipy.constants.hbar)
    )  # + centre_intensity_min
    ...
    sig_times_3 = 3 * np.sqrt(
        (scipy.constants.hbar) / (mass * u * 2 * np.pi * trap_freq * 1e-6)
    )
    ...
    ax_low.text(
        centre_position_um,
        10 - (3 if atom == "Rb" else 0),
        f"TF {trap_freq * 1e3:.1f}(kHz)",
        size="x-small",
        color=col,
        ha="right" if atom == "Cs" else "left",
    )
...
ellipse = mpl.patches.Ellipse(
    xy=(fit_mins[0][0], fit_mins[0][2]),
    width=bare_length_scales_rbcs[0],
    height=bare_length_scales_rbcs[2],
    edgecolor="r",
    fc="None",
    lw=2,
)
...
for sfzi, spacing_from_zero in enumerate(spacing_from_zeros):
    ...
    cfa = ax_2d.contourf(
        x_mesh, y_mesh, pot_mesh, levels=1000, cmap="afmhot_r", norm=norm
    )
    ...
    for si in range(n_copies):
        rbcs_min = bpl.my_find_potential_minimum(
            previous_position[si], tweezer_params_rbcs, "RbCs"
        )
        bare_trap_freqs_khz_rbcs = []
        bare_length_scales_rbcs = []
        ...
        for ci, ax_pair in enumerate(axs.T):
            ...
            wf = (
                mass * u * 2 * np.pi * trap_freq * 1e6 / (np.pi * scipy.constants.hbar)
            ) ** (0.25) * np.exp(
                -(
                    mass
                    * u
            ...
            if si == 0:
                ...
                ax_low.plot(wf_space, wf_prob_per_um, c=col, alpha=0.4)
                ax_low.fill_between(
                    wf_space, wf_prob_per_um, y2=0, color=col, alpha=0.2
                )
    ...
    ellipse = mpl.patches.Ellipse(
        xy=(positions[-1][0][0], positions[-1][0][2]),
        width=lss[-1][0][0],
        height=lss[-1][0][2],
        edgecolor="None",
        fc="green",
    ...
    ellipse_2 = mpl.patches.Ellipse(
        xy=(positions[-1][1][0], positions[-1][1][2]),
        width=lss[-1][1][0],
        height=lss[-1][1][2],
        edgecolor="None",
        fc="green",
