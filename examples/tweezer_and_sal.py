    ...
    for ch_var in xs_values:
        ...
        if plot:
            fig, axs = plt.subplots(
                2, 3, height_ratios=(4, 1), sharey="row", sharex="none"
            )
        for ai, (atom, col) in enumerate(zip(atoms, colours)):
            mass = bpl.species[atom]["m"]
            grad_dec_min = bpl.my_find_potential_minimum(
                (0, 0, 0), tweezer_params, atom
            )
...
axs[1].legend(
    bbox_to_anchor=(0.85, 1.16), ncol=6, fancybox=True, shadow=False, fontsize="x-small"
)
